...
pca_data <- prcomp(data_matrix_orig, center = TRUE, scale. = TRUE, retx = TRUE)
...
head(data_matrix_orig_mds, 5)
```


```{r}
#WORKS
trueN <- data_matrix_orig_mds
...
par(mfrow=c(1,1))
plot(som.model, type="mapping", shape = "straight", border = "grey", labels = data_matrix_orig_mds[,15],col=contrast[trueccolor])
add.cluster.boundaries(som.model, fit_kmeans$cluster, lwd = 3, lty = 2, col=contrast[4]) #name your pallette and choose color...
```

```{r , fig.height=10, fig.width=10}
par(mfrow=c(1,1))
plot(som.model, type="mapping", shape = "straight", border = "grey", labels = data_matrix_orig_mds[,15],col=contrast[haccolor])
...
```{r}
orig_cluster <- data_matrix_orig_mds[,c(11)]
```
