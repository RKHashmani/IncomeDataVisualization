...
clean_sample <- read.csv("data/100framev1.csv")
```
...
First, we try to estimate the best number of clusters, k. Figure \@ref(fig:K-estimate-plot) shows the results of the elbow and silhouette methods. For the elbow method, there seems to be a small bend at k = 10. From the silhouette method, k = 2 seems to be the best choice.
...
##### CHECK CACHE FILE OUTPUT LOOK. IS IT MUSHED?
...
k_clus_10 <- kmeans(data_matrix_scaled, centers = 10, nstart = 100)
...
print(k_clus_5$size)
noquote("Randomly Initialized, k = 10:")
print(k_clus_10$size)
...
```{r k-random, cache=TRUE, fig.ncol = 2, out.width = "50%", fig.align = "center", fig.cap="Results of the k-means clustering for all 4 randomly initialized cluster options. A PCA was produced and the clusterings were projected onto the first 2 principal components in order to visualize them.", fig.subcap=c('k=2', 'k=4', 'k=5', 'k=10')}
...
fviz_cluster(k_clus_10, data = data_matrix_scaled)
...
clara_10 <- clara(data_matrix_clus, k=10, metric = "euclidean", stand = TRUE, samples = 1000, pamLike = TRUE)
```
...
fviz_cluster(clara_10, data = data_matrix_clus)
```
...
```{r SOM, cache=TRUE}
grid_size <- ceiling(sample_number ^ (1/2.5))
som_grid <- somgrid(xdim = grid_size, ydim = grid_size, topo = 'hexagonal', toroidal = T)

som_model <- som(data_matrix_scaled, grid = som_grid, keep.data = TRUE)
```

```{r SOM-plots1, cache=TRUE}
plot(som_model, type="changes")

plot(som_model, type="count", heatkey = TRUE)
...
```
```{r SOM-Plots2, fig.height=10, fig.width=10, cache=TRUE}
plot(som_model)
```
...
cl_assignmentk <- fit_kmeans$cluster[som_model$unit.classif] #we have 100 (since grid=10*10) codes with asigned clusters,
...
#WORKS
colour4 <- tricolor(som_model$grid, phi = c(pi/8, 6, -pi/6), offset = 0.1)
par(mfrow=c(1,1))
plot(som_model, type="mapping", shape = "straight", border = "grey", labels = data_matrix_orig_mds[,11],col=contrast[trueccolor])
add.cluster.boundaries(som_model, fit_kmeans$cluster, lwd = 3, lty = 2, col=contrast[4]) #name your pallette and choose color...
```

```{r , fig.height=10, fig.width=10}
par(mfrow=c(1,1))
plot(som_model, type="mapping", shape = "straight", border = "grey", labels = data_matrix_orig_mds[,11],col=contrast[haccolor])
add.cluster.boundaries(som_model, HACgroups, lwd = 3, lty = 2, col=contrast[4]) #name your pallette and choose color...
```
...
We first perform the nonparametric bootstrap test for our k-means clustering result for 100 bootstraps and k = 2, 4, 5, and 10 (the best options from the previous results). The results for each option are shown below.
...
k_clus_10_boot <- clusterboot(data_matrix_scaled, B=100, bootmethod="boot",
                        clustermethod=kmeansCBI,
                        krange=10, seed=786)
...
clara_8_boot <- clusterboot(data_matrix_scaled, B=100, bootmethod="boot",
                        clustermethod=claraCBI,
                        k=8, seed=786)
clara_10_boot <- clusterboot(data_matrix_scaled, B=100, bootmethod="boot",
                        clustermethod=claraCBI,
                        k=10, seed=786)
...
multi_valid_test <- clValid(data_matrix_scaled, c(2, 3, 4, 5, 8, 10),
...
```{r K-means-10-sil-plot, tidy=FALSE, fig.align = "center", fig.cap="Silhouette plot for the randomly initialized k = 10 k-means cluster." }

sil_k_rand_10 <- silhouette(k_clus_10$cluster, dist_matrix_scaled)

noquote("Cluster Size and Silhouette value for the randomly initialized k-means cluster, k=10.")
fviz_silhouette(sil_k_rand_10, print.summary = TRUE,
               main ="Silhouette Plot (k-means Clustering, randomly initialized, k = 10)")
