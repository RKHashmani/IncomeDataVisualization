...
#Same pre-processing
data_matrix_orig_clus <- data_matrix_orig_mds
data_matrix_clus <- data_matrix_orig_clus[,-c(15)]
...
data_matrix_scaled = scale(data_matrix_clus)
```
...
pca_plot<-my_ggbiplot(pca_data, var.axes=TRUE, groups = data_matrix_orig_mds[,c(15)])
...
# sample_frame <- sample_n(census_total,sample_number)
...
```{r eval=FALSE, echo=FALSE}
#Code to save the figure so I can annotate it: DELETE LATER
...
First, we try to estimate the best number of clusters, k. Figure \@ref(fig:K-estimate-plot) shows the results of the elbow and silhouette methods. For the elbow method, there does not seem to be any apparent "elbow" as the data seems to follow a smooth curve. In an extended version of this graph (not shown), there was no elbow as far as 50 clusters. Nonetheless, we see a slight bend around k = 20 mark and will therefore take k = 20 as one cluster size option. From the silhouette method, k = 5 seems to be the best choice.
...
k_clus_20 <- kmeans(data_matrix_scaled, centers = 20, nstart = 100)
...
print(k_clus_5$size)
noquote("Randomly Initialized, k = 20:")
print(k_clus_20$size)
...
```{r k-random, cache=TRUE, fig.ncol = 2, out.width = "50%", fig.align = "center", fig.cap="Results of the k-means clustering for all 4 randomly initialized cluster options. A PCA was produced and the clusterings were projected onto the first 2 principal components in order to visualize them.", fig.subcap=c('k=2', 'k=4', 'k=5', 'k=20')}
...
fviz_cluster(k_clus_20, data = data_matrix_scaled)
...
```{r}
library(cluster)
library(factoextra) # Duplicated from above
```
...
clara_20 <- clara(data_matrix_clus, k=20, metric = "euclidean", stand = TRUE, samples = 1000, pamLike = TRUE)
```
...
fviz_cluster(clara_20, data = data_matrix_clus)
```

### Self-Organizing maps (SOM) CHECK IF VALIDATION PLOTS ARE CORRECT
...
```{r}
grid.size <- ceiling(sample_number ^ (1/2.5))
som.grid <- somgrid(xdim = grid.size, ydim = grid.size, topo = 'hexagonal', toroidal = T)

som.model <- som(data_matrix_scaled, grid = som.grid, keep.data = TRUE)
```

```{r}
plot(som.model, type="changes")

plot(som.model, type="count", heatkey = TRUE)
...
som.events.colors <- rgb(som.events[,1], som.events[,2], som.events[,3], maxColorValue = 255) # DID NOT WORK
...
```{r}
## generate a plot after training. DID NOT WORK
...
clusterdata <- getCodes(som.model)
wss <- (nrow(clusterdata)-1)*sum(apply(clusterdata,2,var))
for (i in 1:30) wss[i] <- sum(kmeans(clusterdata,
...
plot(1:30, wss, type="b", xlab="Number of Clusters", #match i
...
#WORKS
k = 2
set.seed(786) #for reproducability
fit_kmeans <- kmeans(clusterdata[,1:4], k) #3 clusters are used, as indicated by the wss development.
cl_assignmentk <- fit_kmeans$cluster[som.model$unit.classif] #we have 100 (since grid=10*10) codes with asigned clusters,
#but we have 149 units. The above is to assign units to clusters based on their class-id (code-id) in the SOM model.
clustersKm <- cl_assignmentk #back to original data.
```
```{r}
#WORKS
...
HACgroups <- cutree(fit_hac, k) # cut tree into k clusters
# draw dendrogram with red borders around the 6 clusters 
rect.hclust(fit_hac, k, border="grey")
```

```{r}
#WORKS
cl_assignmenth <- HACgroups[som.model$unit.classif] #we have 100 (since grid=10*10) codes with asigned clusters,
#but we have 149 units. The above is to assign units to clusters based on their class-id (code-id) in the SOM model.
clustersHac <- cl_assignmenth #back to original data.
```

```{r}

head(data_matrix_orig_clus, 5)
```


```{r}
#WORKS
trueN <- data_matrix_orig_clus
kmeancolor <- clustersKm #set color indicator
haccolor <- clustersHac #set color indicator
trueccolor <- trueN #set color indicator
```
```{r}
#WORKS
...
```{r , fig.height=10, fig.width=10}
#WORKS
colour4 <- tricolor(som.model$grid, phi = c(pi/8, 6, -pi/6), offset = 0.1)
par(mfrow=c(1,1))
plot(som.model, type="mapping", shape = "straight", border = "grey", labels = data_matrix_orig_clus[,15],col=contrast[trueccolor])
add.cluster.boundaries(som.model, fit_kmeans$cluster, lwd = 3, lty = 2, col=contrast[4]) #name your pallette and choose color...
```

```{r , fig.height=10, fig.width=10}
par(mfrow=c(1,1))
plot(som.model, type="mapping", shape = "straight", border = "grey", labels = data_matrix_orig_clus[,15],col=contrast[haccolor])
add.cluster.boundaries(som.model, HACgroups, lwd = 3, lty = 2, col=contrast[4]) #name your pallette and choose color...
```


## Validation of Clusterings

Below is a list of all the libraries we use in this section.
...
We first perform the nonparametric bootstrap test for our k-means clustering result for 100 bootstraps and k = 2, 4, 5, and 20 (the best options from the previous results). The results for each option are shown below.
...
k_clus_20_boot <- clusterboot(data_matrix_scaled, B=100, bootmethod="boot",
                        clustermethod=kmeansCBI,
                        krange=20, seed=786)
...
multi_valid_test <- clValid(data_matrix_scaled, c(2, 3, 4, 5, 8, 20),
...
legend("bottomright", clusterMethods(multi_valid_test),col=1:3,
       lty=1:3, pch=paste(1:3), lwd = 3.5)

plot(multi_valid_test, measure=c("AD"), legend=FALSE, lwd = 3.5)
legend("bottomleft", clusterMethods(multi_valid_test), col=1:3,
       lty=1:3, pch=paste(1:3), lwd = 3.5)
...
legend("topleft", clusterMethods(multi_valid_test), col=1:3, lty=1:3,
       pch=paste(1:3), lwd = 3.5)

plot(multi_valid_test, measure=c("Dunn"), legend=FALSE, lwd = 3.5)
legend("topright", clusterMethods(multi_valid_test), col=1:3, lty=1:3,
       pch=paste(1:3), lwd = 3.5)
...
```{r K-means-20-sil-plot, tidy=FALSE, fig.align = "center", fig.cap="Silhouette plot for the randomly initialized k = 20 k-means cluster." }

sil_k_rand_20 <- silhouette(k_clus_20$cluster, dist_matrix_scaled)

noquote("Cluster Size and Silhouette value for the randomly initialized k-means cluster, k=20.")
fviz_silhouette(sil_k_rand_20, print.summary = TRUE,
               main ="Silhouette Plot (k-means Clustering, randomly initialized, k = 20)")
...
```{r}
orig_cluster <- data_matrix_orig_mds[,c(15)]
```
...
|--------------|----------------|------------|
| Hierarchical | DIANA          | 0.0219     |
|              | Ward's Method  | -0.0591    |
|              | GAveraged      | 0.0219     |
| k-means      | Random-Init    | 0.0134     |
|              | Ward-Init      | -0.0649    |
|              | GAveraged-Init | 0.0219     |
|              | DIANA-Init     | 0.0219     |
...
For the k-means cluster estimation, as discussed previously, the elbow method's plot, Figure \@ref(fig:K-estimate-plot), does not show any apparent bend. Nonetheless, given the shape of the plot, we can estimate a slight bend around k = 20. For the silhouette method, while k = 5 seems to be the optimal choice, it should be noted that other values of k also came close, such as k = 3.

Looking at the plots for the randomly initialized k-means clustering, Figure \@ref(fig:k-random), we can see that there is a lot of overlap between the clusters, especially for k values 4, 5, and 20. For k = 4, three clusters can easily be seen, with a 4th one hidden behind. This seems to imply that k = 3 might be a better choice. k = 5 has 4 apparent cluster shapes with one hidden and k = 20 has no discernible cluster shapes at all.
...
From the stability tests, for the nonparametric bootstrap method, we can see that k = 2 had the highest clusterwise Jaccard bootstrap mean for its clusters, the lowest number of dissolved clusters, on average, and its number of recovered clusters closest to the number of bootstraps chosen (100). This indicates that k = 2 clustering is the most stable one given this test. For APN and ADM tests, the DIANA hierarchical clustering with cutoff at k = 2 was the most stable clustering, with values very close to 0 (0.0068 and 0.0323 respectively). However, for the AD and FOM tests, the randomly initialized, k = 20 k-means clustering was more stable, but with slightly larger values (3.4642 and 0.9514, respectively). This indicates that k = 2 clusters and k = 20 clusters seem to be at a tie with respect to the last 4 stability tests.
