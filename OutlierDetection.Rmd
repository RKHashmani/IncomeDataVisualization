...
trsf <- data.frame(predict(dmy, newdata = temp))

head (trsf, 10)

sample <- scale(data.matrix(trsf))
```
...
pca_plot<-my_ggbiplot(pca_data, var.axes=TRUE, groups = sample[,c(85)])
...
elbow_wss <- fviz_nbclust(sample_data, kmeans, method = "wss", k.max=30)
